...
static bool _mic_disabled_logged = false;
...
esp_err_t bsp_extra_codec_set_fs(uint32_t rate, uint32_t bits_cfg, i2s_slot_mode_t ch)
{
    ...
    /* Treat close on an already-closed channel as non-fatal to avoid noisy churn logs. */
    #define MERGE_CODEC_ERR(_expr) do { \
        esp_err_t _e = (_expr); \
        if (_e == ESP_ERR_INVALID_STATE) { \
            _e = ESP_OK; \
        } \
        if (_e != ESP_OK && ret == ESP_OK) { \
            ret = _e; \
        } \
    } while (0)
    ...
    if (play_dev_handle) {
        MERGE_CODEC_ERR(esp_codec_dev_close(play_dev_handle));
    }
    if (record_dev_handle) {
        MERGE_CODEC_ERR(esp_codec_dev_close(record_dev_handle));
        MERGE_CODEC_ERR(esp_codec_dev_set_in_gain(record_dev_handle, CODEC_DEFAULT_ADC_VOLUME));
    }

    if (play_dev_handle) {
        MERGE_CODEC_ERR(esp_codec_dev_open(play_dev_handle, &fs));
    }
    if (record_dev_handle) {
        MERGE_CODEC_ERR(esp_codec_dev_open(record_dev_handle, &fs));
    }

    #undef MERGE_CODEC_ERR
    return ret;
}
...
esp_err_t bsp_extra_codec_reset_state(void)
{
    esp_err_t ret = ESP_OK;
    ...
    if (play_dev_handle) {
        esp_err_t err = esp_codec_dev_close(play_dev_handle);
        if (err != ESP_OK && err != ESP_ERR_INVALID_STATE && ret == ESP_OK) {
            ret = err;
        }
    }

    if (record_dev_handle) {
        esp_err_t err = esp_codec_dev_close(record_dev_handle);
        if (err != ESP_OK && err != ESP_ERR_INVALID_STATE && ret == ESP_OK) {
            ret = err;
        }
    }
...
esp_err_t bsp_extra_codec_init()
{
    ...
    if (play_dev_handle == NULL) {
        ESP_LOGW(TAG, "Speaker codec init failed; audio playback unavailable");
        _is_audio_init = false;
        return ESP_ERR_NOT_FOUND;
    }

    record_dev_handle = NULL;
    if (!_mic_disabled_logged) {
        ESP_LOGI(TAG, "Microphone path disabled for playback-only build");
        _mic_disabled_logged = true;
    }

    esp_err_t fs_err = bsp_extra_codec_set_fs(CODEC_DEFAULT_SAMPLE_RATE, CODEC_DEFAULT_BIT_WIDTH, CODEC_DEFAULT_CHANNEL);
    if (fs_err != ESP_OK) {
        ESP_LOGW(TAG, "Codec sample-rate setup failed: %s", esp_err_to_name(fs_err));
        _is_audio_init = false;
        return fs_err;
    }
