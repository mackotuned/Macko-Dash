...
/**
 * @brief Set soft codec PA active polarity used by fallback speaker path.
 *
 * @param[in] active_high true: PA on when GPIO high, false: PA on when GPIO low
 */
void bsp_audio_soft_codec_set_pa_active_high(bool active_high);

/**
 * @brief Get current soft codec PA active polarity.
 *
 * @return true when PA active-high mode is selected; false for active-low.
 */
bool bsp_audio_soft_codec_get_pa_active_high(void);

/**
 * @brief Set STC8 AUDIO_SD active polarity, if STC8 controller is present.
 *
 * @param[in] active_high true: AUDIO_SD on when high; false: on when low.
 */
void bsp_audio_soft_codec_set_stc8_audio_sd_active_high(bool active_high);

/**
 * @brief Get STC8 AUDIO_SD active polarity setting.
 *
 * @return true for active-high mode, false for active-low mode.
 */
bool bsp_audio_soft_codec_get_stc8_audio_sd_active_high(void);

/**
 * @brief Force PA gate state for soft codec path.
 *
 * @param[in] on true to force PA on; false to force PA off.
 */
