...
#define MY_CODEC_RESET_PIN   (GPIO_NUM_NC)
#define MY_CODEC_PA_CTRL_PIN (BSP_POWER_AMP_IO)
...
static bool s_pa_active_high = false;
...
static int my_codec_open(const audio_codec_if_t *h, void *cfg, int cfg_size)
{
    ...
    codec->gpio_if->set(MY_CODEC_PA_CTRL_PIN, !s_pa_active_high);
    codec->is_open = true;
    return 0;
}
...
{
    s_pa_active_high = active_high;
}

bool my_codec_get_pa_active_high(void)
{
    return s_pa_active_high;
}
